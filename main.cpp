...
static void* thread_func(struct thread_data* thr_data)
{
	...
	uint32_t* const       indices = thr_data->common_data->indices;
	const uint16_t* const table = thr_data->common_data->table;
	uint16_t              value = TABLE_XOR_VAL;
	...
	{
		for (uint32_t index = 0; index < count_of_input_indices; ++index)
		{
			indices[index] = (indices[index] ^ INDEX_XOR_VAL) + thr_data->id;
			const uint32_t table_index = indices[index] & table_index_mask;
			value = (value ^ table[table_index]) & TABLE_ADD_VAL;
		}
	}
